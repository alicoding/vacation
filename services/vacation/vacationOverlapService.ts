...
export async function checkOverlappingBookings(
  ...
  try {
    ...
    console.log(
      `Checking for overlaps with new booking: ${start.toISODate()} to ${end.toISODate()}`,
    );
    ...
    for (const booking of existingBookings) {
      ...
      console.log(
        `Comparing with existing: ${bookingStart.toISODate()} to ${bookingEnd.toISODate()} - Overlap: ${hasOverlap}`,
      );
