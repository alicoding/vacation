...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!session) {
      ...
      return NextResponse.redirect(
        new URL('/auth/signin?error=session_not_established', request.url),
      );
    }
    ...
    console.log(
      'Auth successful, session established for user:',
      session.user.email,
    );

    // Get the callback URL or default to dashboard
    const callbackUrl =
      requestUrl.searchParams.get('callbackUrl') || '/dashboard';
    ...
    if (!siteUrl) {
      console.error(
        'NEXT_PUBLIC_SITE_URL is not set in the environment for the auth callback.',
      );
    ...
    return NextResponse.redirect(
      new URL('/auth/signin?error=unexpected', request.url),
    );
  }
}
