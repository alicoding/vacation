...
let browserClientSingleton: ReturnType<
  typeof createBrowserClient<Database>
> | null = null; // Restore type
...
export const createSupabaseClient = () => {
  ...
  if (isBrowser) {
    if (browserClientSingleton) {
      console.log(
        '[createSupabaseClient] Returning existing browser singleton',
      ); // Add log
  ...
  console.warn(
    'createSupabaseClient() called during server rendering - returning safe placeholder',
  );
      ...
      get: (target, prop) => {
        if (prop === 'auth') {
          return {
            getUser: () => ({
              data: { user: null },
              error: new Error('Client-side auth method called during SSR'),
            }),
            getSession: () => ({
              data: { session: null },
              error: new Error('Client-side auth method called during SSR'),
        ...
        if (typeof prop === 'string' || typeof prop === 'symbol') {
          return () => {
            const error = new Error(
              `Attempted to use Supabase client method '${String(prop)}' during server rendering`,
            );
            console.error(error);
