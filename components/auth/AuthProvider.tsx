...
interface AuthContextType {
  ...
  isAuthProvider: boolean; // Flag to indicate provider presence
...
export const AuthContext = createContext<AuthContextType>({
  ...
  isAuthProvider: false, // Default is false
  signInWithGoogle: () => {
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const initializeAuth = useCallback(async () => {
    try {
      ...
      const timeoutId = setTimeout(() => {
        ...
        if (isLoading) {
          console.warn(
            'Auth detection timeout reached while still loading - forcing state resolution to unauthenticated.',
          );
          ...
          console.log(
            '[AuthProvider] Timeout reached, but loading already finished. No state change forced.',
          );
        }
  ...
  const contextValue = useMemo(
    () => ({
      ...
      isLoading,
      isAuthenticated,
      isAuthProvider: true, // Set flag to true when provider is used
      signInWithGoogle,
      signOut,
      refreshSession,
    }),
