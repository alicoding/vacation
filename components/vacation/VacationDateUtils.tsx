...
export const generateBankHolidayMap = (
  holidays: any[],
): Map<string, string> => {
  const map = new Map();
...
export const createRenderDay = ({
  bankHolidayMap,
  existingVacations,
}: RenderDayProps) => {
  // Custom day renderer for DatePicker to highlight holidays and booked days
  const CustomDayRenderer = (props: PickersDayProps<DateTime>) => {
    ...
    if (
      ...
      console.log(
        'Processing vacation dates in renderDay:',
    ...
    const isVacation = existingVacations.some((vacation) => {
      ...
      if (
        normalizedDay.day === 15 &&
        normalizedDay.month === DateTime.now().month
      ) {
        console.log(
          'Checking day:',
          normalizedDay.toISODate(),
          'against vacation:',
          startDate.toISODate(),
          'to',
          endDate.toISODate(),
    ...
    if (isHoliday) {
      tooltipTitle =
        bankHolidayMap.get(normalizedDay.toUTC().toFormat('yyyy-MM-dd')) ||
        'Holiday';
    } else if (isVacation) {
  ...
  CustomDayRenderer.displayName = 'CustomDayRenderer';
  return CustomDayRenderer;
};
